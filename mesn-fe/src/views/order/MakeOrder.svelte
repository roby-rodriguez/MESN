<script>
  ...
  onMount(async () => {
    ...
    try {
      ...
      productSearch = myProducts;
    } catch (error) {
      console.log(error);
    }
  });

  function search(productInSearch) {
    productSearch = myProducts.filter((product) =>
      product.name.includes(productInSearch)
    );
  }
  ...
  $: {
    ...
    order.total = Math.floor(sum * 100) / 100;
  }
...
<div class="grid grid-cols-1 md:grid-cols-2 col-gap-20 row-gap-12 p-8">
  <div>
    <TextField
    ...
    <table class="table-fixed">
      <thead>
        <tr>
          ...
          <th class="w-1/2 px-4 py-2">Unit price</th>
          <th class="w-1/6 px-4 py-2">Add</th>
        </tr>
      </thead>
      <tbody>
        {#each productSearch as product}
          <tr>
  ...
  <div class="col-span-1">
    ...
    <form on:submit|preventDefault={makeOrder}>
      <table class="table-fixed">
        ...
        <tbody>
          {#each order.products as { _id, name, quantity, uprice, amount }, index}
            <tr>
              <td class="border px-4 py-2">{name}</td>
              <td class="border px-4 py-2">
                <input
                  bind:value={quantity}
                  on:input={() => (amount = Math.floor(quantity * uprice * 100) / 100)}
                  type="number"
                  min="1"
                  required
                  class="w-1/2" />
      ...
      <div class=" m-10 grid grid-cols-2">
        ...
        <Button type="submit" class="m-auto" color="dark">
          generate
        </Button>
      </div>
    </form>
